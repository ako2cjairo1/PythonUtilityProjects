...
MUSIC_DB_FILE = "C:\\Users\\Dave\\DEVENV\\Python\\PythonUtilityProjects\\Music Database.json"
PLAYER_SETTING = "C:\\Users\\Dave\\DEVENV\\Python\\PythonUtilityProjects\\Player Control.json"
...
class MusicPlayer:
    ...
    def get_duration(self, media=None, ticker=0):
        ...
        seconds = int("{:.0f}".format(
            get_length - (10 if get_length > 0 else 0))) - ticker
    ...
    def search_song_by(self, title="", artist="", genre=""):
        def _isFound(song):
            ...
            title_filter = (title.strip().lower()
                            in song["title"].strip().lower())
            artist_filter = (artist.strip().lower()
                             in song["artist"].strip().lower())
            genre_filter = (genre.strip().lower()
                            in song["genre"].strip().lower())

            if title and artist and genre:
                if not title_filter:
                    title_filter = _is_possible_match(
                        title, song["title"].strip().lower())
                if not artist_filter:
                    artist_filter = _is_possible_match(
                        artist, song["artist"].strip().lower())
                if not genre_filter:
                    genre_filter = _is_possible_match(
    ...
    def load_media(self, is_playlist=True):
        ...
        def _update_music_db():
            ...
            if not os.path.isfile(MUSIC_DB_FILE):
                for item in playlist:
                    ...
                    song = Song(item["name"], item["track"], item["title"], item["artist"],
                                item["genre"], item["duration"], item["status"], item["song_dir"]).serialize()
        ...
        def _get_metadata(filename, subdir):
            ...
            if _is_audiofile(filename):
                ...
                try:
                    ...
                    title = "" if media.get_meta(
                        0) is None else media.get_meta(0)
                    # artist from meta data
                    artist = "" if media.get_meta(
                        1) is None else media.get_meta(1)
                    # artist from meta data
                ...
                if title and (track_no or artist or genre):
                    song = Song(filename, track_no, title, artist,
                                genre, duration, status, subdir)
        ...
        if len(playlist) > 0:
            ...
            # from the playlist, filter the "song_dir"
                ...
                playlist = [songs for songs in playlist if songs["song_dir"].strip(
                ).lower() == str(self.folder_dir).strip().lower()]
            ...
            # 1 song loader, check if audio file exist
                ...
                playlist = [song for song in playlist if song["name"].strip().lower() == song_name.strip(
                ).lower() and song["song_dir"].strip().lower() == song_dir.strip().lower()]
        ...
        if len(playlist) <= 0:
            ...
            if self.all_music:
                ...
                for subdir, _, music_files in os.walk(Path.home()):
                    ...
                    music_files = [
                        music for music in music_files if _is_audiofile(music)]
            ...
            # random music loader
                ...
                for subdir, _, music_files in os.walk(Path.home()):
                    ...
                    if len(music_files) > 0:
                        ...
                        songs = [Song(name, 0, "", "", "", "", "",
                                      subdir).serialize() for name in music_files]
                ...
                if len(all_song) > 0:
                    ...
                    while True:
                        ...
                        if not self.isFound(rand_music, random_song["name"], random_song["song_dir"]):
                            ...
                            _get_metadata(
                                random_song["name"], random_song["song_dir"])
                            music_counter += 1
        ...
        if self.title or self.artist or self.genre:
            filtered_playlist = self.search_song_by(
    ...
    def music_player(self, playlist_folder="", play_shuffle=False, play_all_music=False):
            ...
            def _show_playlist(status):
                ...
                def _playing_status(state):
                    ...
                    if self.player.get_state() == state.Playing:
                        left_arrows = str(('<' * arrow_counter)
                                          if arrow_counter > 0 else "").rjust(3)
                        right_arrows = str(
                            ('>' * arrow_counter) if arrow_counter > 0 else "").ljust(3)
                ...
                # playlist header
                print(
                    f" #    {'Title'.ljust(TITLE_PADDING)} {'Artist'.ljust(ARTIST_NAME_PADDING)} Time      Status")
                print(" " + "=" * (TITLE_PADDING + ARTIST_NAME_PADDING + 27))

                total = len(self.playlist)
                current_song_idx = self.playlist.index(
                    ([song for song in self.playlist if current_song in song["name"]][0]))
                ...
                for song in self.playlist[alpha:omega]:
                    ...
                    title = f"{song['title'] if len(song['title']) < TITLE_PADDING else song['title'][:(TITLE_PADDING - 4)].strip() + '...'}".ljust(
                        TITLE_PADDING)
                    artist = f"{song['artist'] if len(song['artist']) < ARTIST_NAME_PADDING else song['artist'][:(ARTIST_NAME_PADDING - 3)].strip() + '...'}".ljust(
                        ARTIST_NAME_PADDING)
                    ...
                    if song['name'] == current_song and (status.Playing or status.Paused):
                        ...
                        print(" " + "-" * (TITLE_PADDING +
                                           ARTIST_NAME_PADDING + 27))
                        print(
                            f" {track_no} {title} {artist} {duration}  {_playing_status(status)}")
                        print(" " + "-" * (TITLE_PADDING +
                                           ARTIST_NAME_PADDING + 27))
                ...
                if len(self.playlist) > 25:
                    print(
                        f" Showing {len(self.playlist[alpha:omega])} of {len(self.playlist)} songs on the playlist...")
                else:
        ...
        if self.toggle_shuffle:
            ...
            for _ in self.playlist:
                unplayed_list = [
                    song for song in self.playlist if song["status"] == ""]
        ...
        # ordered playlist
            ...
            self.playlist = sorted(
                self.playlist, key=lambda x: int(x["track"]))
                ...
                for song in self.playlist:
                    if music_name and music_name == song["name"]:
                        ...
                        if status.Playing or status.Paused:
                            ...
                            animate_track_details = track_details[int(scroll_ticker * 10):(len(track_details) - 1) + int(
                                scroll_ticker * 10)] + track_details[0:int(scroll_ticker * 10)]

                            print(" " + "-" * (TITLE_PADDING +
                                               ARTIST_NAME_PADDING + 33))
            ...
            if self.toggle_shuffle:
                ...
                for _ in self.playlist:
                    ...
                    if len(unplayed_list) > 0:
                        ...
                        # play the song
                        (is_done_playing, song_status) = _play(
                            song_choice["name"])
                        ...
                        # update status of the song in self.playlist (marked as "Done", "Skipped" or "Can't play")
                        self.update_playlist(
                            song_choice["name"], key="status", value=song_status)
    ...
    def play_some_music(self, is_playlist=True, playlist_folder="", music_file="", play_all_music=False, shuffle=False, compact_mode=True):
        if compact_mode:
            self.compact_music_player(is_playlist=is_playlist, playlist_folder=playlist_folder,
                                      music_file=music_file, play_shuffle=shuffle, play_all_music=play_all_music)
        else:
            self.music_player(
                play_shuffle=shuffle, playlist_folder=playlist_folder, play_all_music=play_all_music)
        return


if __name__ == "__main__":
    parser = ArgumentParser(
        description="Give option to search and play music.")

    # add option argument
    parser.add_argument("--option", action="store", dest="option",
                        required=True, help="Command option for music player.")

    # required parm for option "music file"
    parser.add_argument("--filename", action="store",
                        dest="filename", help="Filename of music to play.")

    # required parm for option "playlist"
    parser.add_argument("--dir", action="store", dest="dir_folder",
                        help="Folder location of playlist to play.")

    # shuffle option
    parser.add_argument("--shuffle", action="store", dest="isShuffle",
                        help="Shuffle the playlist. [True or False]")

    # Music player view (compact or normal)
    parser.add_argument("--mode", action="store", dest="isCompact",
                        help="Determine the view of Music player [compact or normal].")

    # required parms for option: "play by"
    parser.add_argument("--title", action="store", dest="title",
                        help="Title of music to search and play.")
    parser.add_argument("--artist", action="store", dest="artist",
                        help="Artist name of music to search and play.")
    parser.add_argument("--genre", action="store", dest="genre",
                        help="Genre of music to search and play.")
    ...
    # --mode parameter
    isCompact = True if param.isCompact and param.isCompact.strip(
    ).lower() == "compact" else False

    try:
        if option == "music file":
            # play sing song in compact mode
            mp.play_some_music(is_playlist=False, playlist_folder=param.dir_folder,
                               music_file=param.filename, shuffle=isShuffle, compact_mode=isCompact)

        elif option == "playlist":
            # play the playlist of music
            mp.play_some_music(playlist_folder=param.dir_folder,
                               shuffle=isShuffle, compact_mode=isCompact)

        elif option == "play all":
            # play all music starting from User folder directory
            mp.play_some_music(play_all_music=True, playlist_folder=Path.home(
            ), shuffle=isShuffle, compact_mode=isCompact)
